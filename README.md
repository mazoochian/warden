...
- Live Answers: Replies to your questions arrive as a threaded reply that updates in place — a "🤔 Thinking..." header with a live tree of what's happening underneath it, then editing into the final answer. Each tool gets its own icon (`🌐 Using web_search`, `🪒 Using scrape_site`, `🧠 Using ask_delegate`, …). The same tool called again with *different* arguments collapses into a count (`🌐 Using web_search (x3)`) rather than repeating the line, and the model re-issuing an identical call adds nothing at all. A model call that fails for a transient reason (timeout, dropped connection, a 429/5xx during busy hours) is retried automatically with exponential backoff, shown as `🔄 API Error: Retrying (1/3)` — set `WARDEN_LLM_MAX_RETRIES` (default 3, or the `WARDEN_LLM_MAX_RETRIES` dynamic-config key) to change it, or 0 for the old single-attempt behaviour. Only transport-level failures are retried; a malformed request or a bad key fails immediately rather than three times. Each chat's messages are handled independently and concurrently, so one slow or stuck reply never blocks the rest of the bot
- Curated Feed (Telegram, owner-only): a personal, self-managing news feed. `/feed add <channel name or id>` watches a channel through your *personal* account (so any channel you can read works, not just ones a bot could join), `/feed policy <text>` says what you actually want in plain language ("international news, nothing about crypto prices"), `/feed target <chat id>` picks where digests get posted, and `/feed every 1h` sets the cadence. Every new post is checked against your policy with a cheap one-word relevance call; only what passes is summarized into a one-line entry, and the survivors are posted as a single grouped digest per interval rather than a stream of forwards. `/feed list`, `/feed remove`, `/feed on`/`/feed off` and `/feed run` (a pass right now) manage it, and the web UI's Curated Feed page does all of the same. Sources are opt-in rather than "every channel you follow" — that keeps the model bill a function of what you asked for rather than how many channels you happen to be in — and a newly added channel is caught up silently, so adding one never dumps its backlog into your feed. The feed stays completely inert until both a target and a policy are set: it will never fall back to forwarding everything
- Long-Term Memory: ask Warden to remember something ("remember that I prefer short answers") and it keeps it as a per-identity fact that follows you across every chat, not just the one you said it in. `/memory list` shows what it has, `/memory forget <id>` drops one (only the person a memory belongs to can forget it), and the web UI's Memory page does both. Facts are ranked into the prompt by a blend of keyword match, recency and how often they've been confirmed. **An embeddings endpoint is optional**: set `WARDEN_EMBEDDINGS_URL` (plus `WARDEN_EMBEDDINGS_API_KEY`/`WARDEN_EMBEDDINGS_MODEL`, output dimension must be 1536) to additionally get semantic, meaning-based recall; without it everything still records and recalls, just on the other three signals. Warden logs which mode it started in
- Chain-of-thought: when a reasoning model's thinking is shown (`/thinking on`), it renders as a collapsible 💭 blockquote on Telegram. Anywhere that can't do that — Matrix, XMPP, or a Telegram message whose HTML formatting the API rejected — it degrades to a plain `💭 …` paragraph instead of leaking the internal markers as unformatted text
...
- Personal Account (Telegram, owner-only): once `WARDEN_TELEGRAM_USER_*` is configured (see below), `/tdlogin` logs Warden into the *owner's own* Telegram account via TDLib (phone number → login code → 2FA password if set), separate from the bot account every other feature uses. `/tdchats` lists that account's known chats; `/sendas <chat id> <text>` sends through it manually. `/autonomy off|draft|auto` sets the reply mode per chat: `draft` has the bot write a reply and hold it for approval, `auto` sends it immediately. A drafted reply is **typed straight into that chat's Telegram composer**, so opening the conversation on your phone or desktop shows it already there, ready to edit or send — Telegram syncs drafts across your own devices, so this works wherever you actually read the message. Anything you'd already typed in that composer is overwritten, but never silently: it's quoted back to you in the notification and shown on the drafts page. You also get an Approve/Discard notification (`/approve`/`/discard <chat id>`, or `/drafts` to list, still work if typed instead, as does the web UI's Personal Account page); approving sends the draft and clears the composer, discarding just clears it. Drafts are stored in Postgres, so they survive a restart or redeploy — a draft you were told about is still there hours later. Note that reply autonomy is the *only* thing an incoming message on the personal account can trigger: a contact messaging you can never run a bot command through your own account, whatever they type. `/autonomy <chat id> prompt [<text>|off]` sets that chat's ghostwriter voice — kept separate from `/persona` on purpose, since `/persona` styles Warden answering as *itself* while this one styles it impersonating you; by default it writes in your voice with no AI framing. `/tdsummary <chat id or name>` (or just ask, e.g. "what's unread in the Alice chat") fetches that chat's actual unread messages fresh from Telegram, has the bot summarize them, and marks them read — matching titles work as well as raw ids, and an ambiguous name lists the candidates instead of guessing
