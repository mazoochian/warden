...
- Announcements: `/announce <time> <text>` schedules a broadcast into this chat, `/announce every <interval> <text>` repeats one (same time grammar as `/remind`); `/announce list` / `/announce cancel <id>` manage them. Chat admins only — unlike `/remind`, this makes the bot post to the whole group when nobody's watching. `/autopin on` additionally pins each announcement as it's posted (off by default; the bot needs pin permission). Auto-pin only ever pins announcements an admin scheduled — the bot never decides on its own that someone else's message is "important"
...
- Video Auto-Download: `/videodownload on` makes the bot watch for YouTube/Instagram/X (Twitter) video links posted in the chat and auto-fetch/repost them as a file via `yt-dlp`, capped at Telegram's cloud Bot API's real 50MB upload ceiling. Best-effort and fails closed — age-restricted, private, geo-blocked, oversized, or otherwise undownloadable links are silently skipped, never an in-chat error. Off by default (chat admins only to enable); not supported on XMPP (no file-sending capability there at all)
...
- Management Rooms: bind a private room to the chats you administer with `/manage bind <chat id>` (`/manage list` shows the bindings, `/manage unbind` removes one), then drive those chats from it without joining them in a client. `/notice <chat id> <text>` sends a pinned notice; `/as <chat id> <command>` runs an admin command against a bound chat — `/as 7 /kick @spammer`, `/as 7 /welcome hi {name}`, `/as 7 /stats` — and the reply comes back to you in the management room rather than into the target chat. `/as` is strictly narrowing: it re-checks on every use that you're a live admin *of the target chat* (never inferred from your standing in the management room), and the relayed command still runs its own permission check too, so it can never let you do something typing the same command in that chat wouldn't have. Commands needing a message to point at (`/pin`, `/delete`, `/redact reply`) and stateful flows (`/menu`, `/convert`) are deliberately not relayable
