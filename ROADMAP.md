...
- **Merged into master 2026-08-09**: this phase's migration was originally
  numbered `0037_member_acl.sql` (picked independently in this phase's own
  worktree) but collided with two migrations Phase 20 added under the same
  number in master meanwhile — renumbered to `0039_member_acl.sql` at
  merge time, `migrate.zig`'s registration updated to match, no SQL content
  changed.
