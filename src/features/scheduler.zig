...
pub const BriefingScheduler = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, io: Io, interval_seconds: i64) BriefingScheduler {
        return .{
            .allocator = allocator,
            .io = io,
            .enabled_chats = std.StringHashMap(void).init(allocator),
    ...
    pub fn deinit(self: *BriefingScheduler) void {
        var it = self.enabled_chats.keyIterator();
        while (it.next()) |k| self.allocator.free(k.*);
        self.enabled_chats.deinit();
    }
    ...
    pub fn enable(self: *BriefingScheduler, platform: Platform, native_chat_id: []const u8) !void {
        const key = try compositeKey(self.allocator, platform, native_chat_id);
        errdefer self.allocator.free(key);
    ...
    pub fn disable(self: *BriefingScheduler, allocator: std.mem.Allocator, platform: Platform, native_chat_id: []const u8) void {
        const key = compositeKey(allocator, platform, native_chat_id) catch return;
        defer allocator.free(key);
    ...
    pub fn isEnabled(self: *BriefingScheduler, allocator: std.mem.Allocator, platform: Platform, native_chat_id: []const u8) bool {
        const key = compositeKey(allocator, platform, native_chat_id) catch return false;
        defer allocator.free(key);
    ...
    pub fn snapshotEnabledChatIds(self: *BriefingScheduler, allocator: std.mem.Allocator) ![]ChatKey {
        self.mutex.lockUncancelable(self.io);
        defer self.mutex.unlock(self.io);
...
test "BriefingScheduler: enable/disable/isEnabled" {
    var sched = BriefingScheduler.init(testing.allocator, testing.io, 86400);
    defer sched.deinit();
    const a = testing.allocator;
...
test "BriefingScheduler: the same native chat id on two different platforms doesn't collide" {
    var sched = BriefingScheduler.init(testing.allocator, testing.io, 86400);
    defer sched.deinit();
    const a = testing.allocator;
...
test "BriefingScheduler: snapshotEnabledChatIds recovers platform and native id from the composite key" {
    var sched = BriefingScheduler.init(testing.allocator, testing.io, 86400);
    defer sched.deinit();
    const a = testing.allocator;
