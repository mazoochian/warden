...
pub fn create(pool: *PgPool, chat_id: i64, identity_id: i64, message: []const u8, due_at: i64) !i64 {
    const db = try pool.acquire();
    defer pool.release(db);

    var stmt = try db.prepare(
        \\INSERT INTO reminders (chat_id, identity_id, message, due_at)
...
pub fn listPending(pool: *PgPool, allocator: std.mem.Allocator, chat_id: i64) ![]PendingReminder {
    ...
    var stmt = try db.prepare(
        \\SELECT id, message, EXTRACT(EPOCH FROM due_at)::bigint
...
test "create/dueUndelivered/markDelivered/listPending/get/cancel" {
    ...
    const id1 = try create(&pool, chat_id, identity_id, "take out trash", 2000);
    const id2 = try create(&pool, chat_id, identity_id, "future thing", 9999);
